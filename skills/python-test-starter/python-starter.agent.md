...
- **Security**: no hardcoded secrets/URLs — read from the `TEST_CONFIG` registry or env. Never log credentials or PII. No `shell=True`. Secrets are stripped at import.
